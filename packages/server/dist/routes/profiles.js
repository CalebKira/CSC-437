...
router.post("/", (req, res) => {
  ...
  import_profile_svc.default.create(newProfile).then(
    (profile) => res.status(201).json(profile)
  ).catch((err) => res.status(500).send(err));
});
router.put("/:userid", (req, res) => {
  const { userid } = req.params;
  const newUser = req.body;
  import_profile_svc.default.update(userid, newUser).then((profile) => res.json(profile)).catch((err) => res.status(404).end());
});
